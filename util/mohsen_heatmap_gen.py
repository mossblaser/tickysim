...
"""
Reads in TSVs of pages of Mohsen's heatmap results and produces a TSV formatted
as the heatmap generator would enjoy.

Usage:
mohsen_heatmap_gen.py 8 8 some_stuff_cyclic.tsv some_stuff_tornado.tsv some_stuff_transpose.tsv some_stuff_complement.tsv
"""
...
def extract_runs(raw_data):
	...
	Given a TSV file as a string, returns a dictionary::
		...
		{inj_period: [(dropped, injected, routed, received)], ...}
	"""
	...
	for row in rows:
		...
		for run_cols_start in range(1, len(row), 4):
			inj_period = int(rows[0][run_cols_start])
...
def render_heatmap_file(patterns, width, height, link_latency=16):
	...
	as produced by extract_runs(), make a TSV file with the following columns:
		...
		chip_x, chip_y, dropped, injected, routed, received, inj_rate, inj_period, pattern
	
	Also takes the width and height of the system.
	"""
	
	out = "chip_x\tchip_y\tdropped\tinjected\trouted\treceived\tinj_rate\tinj_period\tpattern\n"
	
	for pattern_name, runs in patterns.iteritems():
		for inj_period, run in runs.iteritems():
			for core_id, (dropped, injected, routed, received) in enumerate(run):
